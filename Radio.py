...
class RadioRunner:
    def __init__(self, text_channel, bot_loop):
        ...
        self.stt_q = queue.Queue(maxsize=STT_QUEUE_MAX)
    ...
    def stt_worker(self):
        try:
            ...
            self.send_text_to_discord(f"Whisper model `{STT_MODEL_NAME}` loaded.")
        except Exception as e:
            self.send_text_to_discord(f"Whisper load error: `{e}`")
            return
        ...
        while not self.stop_event.is_set():
            try:
                mono_chunk = self.stt_q.get(timeout=0.2)
            except queue.Empty:
                continue
            ...
            try:
                ...
                if text:
                    self.send_text_to_discord(text)

            except Exception as e:
                self.send_text_to_discord(f"STT error: `{e}`")
